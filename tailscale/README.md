...
eg:- if using tailscale for the searxng then add searxng compose contents in the tailscale compose file and rename the file as ts-searxng.yml  
...
- Run with  
`docker compose -f ts-searxng.yml -p ts-searxng up -d`  
- To stop the container  
`docker compose -f ts-searxng.yml -p ts-searxng down`  
